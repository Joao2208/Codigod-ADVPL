...
User Function MM561() 
    ...
    if "ZB4Z6T_PRD" $ cAmbName  
        If IsBlind()
            Aviso("Rotina não permitida no ambiente de Producao",cMsg,{"Ok"},2)
        Else
            Help(,,"Rotina não permitida no ambiente de Producao",,cMsg,1,0)
        EndIf
        Return
    endif
    ...
    for nCount := 1 to Len(aAllGroup) 
        ...
        //Pega todos os parametros que estão no cDelet e que possuirem as Palavras MADEIRA E BULKYLOG no conteudo
        while !SX6->(EOF())
            cParVal := SX6->X6_CONTEUD
            cParName := SX6->X6_VAR
            cFil := SX6->X6_FIL
            if VALTYPE(cParVal) == "C"
                if 'MADEIRA' $ UPPER(cParVal) .or. 'BULKYLOG' $ UPPER(cParVal) .or. AllTrim(Upper(cParName)) $ cDelet
        ...
        //Limpa e deleta os parametros encontrados acima
        for nCol := 1 to Len(aParam)
            SX6->(DbSeek(aParam[nCol][1] + aParam[nCol][2]))
            if 'MADEIRA' $ Upper(GetMV(aParam[nCol][2])) .or. 'BULKYLOG' $ Upper(GetMV(aParam[nCol][2]))
                RecLock("SX6", .F.)
	    	        SX6->X6_CONTEUD := ""
                    SX6->X6_CONTSPA := ""
                    SX6->X6_CONTENG := ""
	            SX6->(MsUnlock())
            elseif aParam[nCol][2] $ cDelet
                RecLock("SX6", .F.)
                    SX6->(DBDelete())
                SX6->(MsUnlock())
            endif         
        ...
        //Fecha a tabela e limpa o ambiente
        DbCloseArea()
        RpcClearEnv()

        //Conecta no ambiente denovo
        RPCSetType(3)
        lEnv := RPCSetEnv(aAllGroup[nCount], aAllGroup[nCount] + '0101')
        IF !lEnv
            ConOut("MM561 - Não conseguiu preparar ambiente")
        ...
        cQuery := "UPDATE " + RetSqlNAme("SRH") + " SET RH_SALMES = RH_SALMES * 0.023, RH_SALDIA = RH_SALDIA * 0.023, RH_SALHRS = RH_SALHRS * 0.023, RH_SALARIO = RH_SALARIO * 0.023"
        TCSqlExec(cQuery)
        
        //SRG - RG_NORMAL, RG_DESCANS, RG_SALMES, RG_SALDIA, RG_SALHORA (Cabeçalho Rescisão)
        cQuery := "UPDATE " + RetSqlNAme("SRG") + " SET RG_NORMAL = RG_NORMAL * 0.023, RG_DESCANS = RG_DESCANS * 0.023, RG_SALMES = RG_SALMES * 0.023, RG_SALDIA = RG_SALDIA * 0.023, RG_SALHORA = RG_SALHORA * 0.023"
        TCSqlExec(cQuery)
